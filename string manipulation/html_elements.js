...
function HTMLElements(string) {
    ...
    for(let i=0; i<openingTags.length; i++){
        temp1 = new String("");
        for(let j=0; j<openingTags[i].length; j++) if(openingTags[i][j]!=='<' && openingTags[i][j]!=='>') temp1 += openingTags[i][j];
        for(let m=0; m<closingTags.length; m++) {
            temp2 = new String("");
            for(let n=0; n<closingTags[m].length; n++) if(closingTags[m][n]!=='<' &&closingTags[m][n]!=="/" && closingTags[m][n]!=='>') temp2 += closingTags[m][n];
            if(temp1===temp2) {
