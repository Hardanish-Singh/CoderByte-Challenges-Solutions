...
function RectangleArea(strArray) {
    let A = strArray[0]
        .slice(1)
        .slice(0, strArray[0].length - 2)
        .split(" ");
    let B = strArray[1]
        .slice(1)
        .slice(0, strArray[1].length - 2)
        .split(" ");
    let C = strArray[2]
        .slice(1)
        .slice(0, strArray[2].length - 2)
        .split(" ");
    let D = strArray[3]
        .slice(1)
        .slice(0, strArray[3].length - 2)
        .split(" ");
    ...
    return (
        (Math.max.apply(Math, array1) - Math.min.apply(Math, array1)) *
        (Math.max.apply(Math, array2) - Math.min.apply(Math, array2))
    );
}
